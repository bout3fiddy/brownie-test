...
try:
    ...
    DecimalOverrideException = (
        BaseException  # regular catch blocks shouldn't catch
    )
...
class Wei(int):
    ...
    def to(self, unit: str) -> "Fixed":
        ...
        try:
            ...
            raise TypeError(
                f'Cannot convert wei to unknown unit: "{unit}". '
            ) from None
...
def _to_wei(value: WeiInputTypes) -> int:
    ...
    if isinstance(value, float) and "e+" in str(value):
        ...
        return int(
            num[0] + num[1][: int(dec)] + "0" * (int(dec) - len(num[1]))
        )
...
class Fixed(Decimal):
    ...
    def __eq__(self, other: Any) -> bool:  # type: ignore
        if isinstance(other, float):
            raise TypeError(
                "Cannot compare to floating point - use a string instead"
            )
        try:
...
def _to_fixed(value: Any) -> Decimal:
    if isinstance(value, float):
        raise TypeError(
            "Cannot convert float to decimal - use a string instead"
        )
...
def _address_compare(a: Any, b: Any) -> bool:
    ...
    if not b.startswith("0x") or not eth_utils.is_hex(b) or len(b) != 42:
        raise TypeError(
            f"Invalid type for comparison: '{b}' is not a valid address"
        )
