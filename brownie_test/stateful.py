    ...
    def __init__(self) -> None:
        ...
        if capman:
            with capman.global_and_fixture_disabled():
                c = "red" if self._failed else "yellow"
...
def _member_filter(member: tuple) -> bool:
    ...
    return (
        ...
        and not next(
            (
                i
                for i in fn.__dict__.keys()
                if i.startswith("hypothesis_stateful")
            ),
            False,
        )
    )
...
def _generate_state_machine(rules_object: type) -> type:
    ...
    strategies: Dict = {
        k: v
        for k, v in getmembers(rules_object)
        if isinstance(v, SearchStrategy)
    }

    for attr, fn in filter(_member_filter, getmembers(machine)):
        varnames = [
            [i] for i in fn.__code__.co_varnames[1 : fn.__code__.co_argcount]
        ]
        if fn.__defaults__:
